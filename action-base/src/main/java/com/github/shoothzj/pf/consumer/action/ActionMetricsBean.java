...
public class ActionMetricsBean {
    ...
    public ActionMetricsBean(MeterRegistry meterRegistry, ActionType actionType) {
        ...
        this.failCounter = meterRegistry.counter(FAIL_COUNT_NAME_FORMAT, tags);
        this.successSummary = meterRegistry.summary(SUCCESS_LATENCY_SUMMARY_NAME_FORMAT, tags);
        this.failSummary = meterRegistry.summary(FAIL_LATENCY_SUMMARY_NAME_FORMAT, tags);
        this.successTimer = Timer.builder(SUCCESS_LATENCY_TIMER_NAME_FORMAT)
                .publishPercentiles(0.5, 0.75, 0.9, 0.95, 0.99, 0.999)
                .publishPercentileHistogram(true)
                .tags(tags).register(meterRegistry);
        this.failTimer = Timer.builder(FAIL_LATENCY_TIMER_NAME_FORMAT)
                .publishPercentiles(0.5, 0.75, 0.9, 0.95, 0.99, 0.999)
                .publishPercentileHistogram(true)
                .tags(tags).register(meterRegistry);
    }
